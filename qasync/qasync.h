...
#define CallbackInstance CallbackSignal::getInstance()
    ...
    struct Promise {
        ...
        bool ready = false;
        std::coroutine_handle<> after_await;
    };
    ...
    struct Awaitable {
        ...
        auto await_suspend(std::coroutine_handle<> callback_handle) noexcept
        {
            ...
            this->_handle.promise().after_await = callback_handle;
...
//        t.detach();
    ...
    struct toMainThread {
        ...
        void await_suspend(std::coroutine_handle<> coro)
        {
            std::cout << NOW_TIME_T << "\t" << __FUNCTION__ << " toMainThread my thread id is " << std::this_thread::get_id() << std::endl;
            emit CallbackSignal::getInstance()->awaitHandleResume(coro.address());
            //        std::thread t(
            //        [=] {
            //            std::cout<< NOW_TIME_T << "\t" << "New thread id " << std::this_thread::get_id() << std::endl;
            //            coro.resume();
